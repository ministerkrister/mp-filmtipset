...
namespace Filmtipset.GUI
{
    internal class Gui2UtilConnector
    {
        ...
        System.Timers.Timer timeoutTimer = new System.Timers.Timer(FilmtipsetSettings.WebRequestTimeout) { AutoReset = false };
        ...
        void StopBackgroundTask(bool byUserRequest)
        {
            ...
            {
                Log.Info("[Filmtipset] Aborting background thread{0}.", byUserRequest ? " by User Request" : "");
        ...
        internal bool ExecuteInBackgroundAndCallback(Func<object> task, Action<bool, object> resultHandler, string taskDescription, bool timeout)
        {
            ...
            if (!IsBusy && Monitor.TryEnter(this))
            {
                try
                {
                    ...
                    backgroundThread = new Thread(delegate()
                    {
                        ...
                        {
                            if (!abortedByUser) Log.Warn("[Filmtipset] Timeout waiting for results.");
                            Thread.ResetAbort();
                        }
                        catch (Exception threadException)
                        {
                            _CurrentError = threadException;
                            Log.Warn(string.Format("[Filmtipset] {0}",threadException.ToString()));
                    ...
                    }) { Name = "Filmtipset", IsBackground = true };
                ...
                {
                    Log.Error(string.Format("[Filmtipset] {0}", ex));
                    IsBusy = false;
            ...
            {
                Log.Error("[Filmtipset] Another thread tried to execute a task in background.");
                return false;
            }
        }
        ...
        void ExecuteTaskResultHandler()
        {
            ...
            if (_CurrentTaskSuccess != true)
            {
                if (_CurrentError != null)
                {
                    ...
                    {
                        dlg_error.Reset();
                        dlg_error.SetHeading("Filmtipset");
